...
export const estimateLiquidityTransaction = createAsyncThunk(
  ...
  async (data: LiquidityTxRequestInterface, thunkAPI) => {
    ...
    const getLiquidityTxEstimationPromise = PromiseUtils.makeCancelable<LiquidityTxInterface>(
      swapService.getLiquidityTxEstimation(data, state.app.settings),
    );
...
export const getLiquidityPool = createAsyncThunk(
  ...
  async (search: { token0: string; token1: string }, thunkAPI) => {
    ...
    const walletAdapterService = state.wallet.adapter;
    const tokens: TokenInterface[] = state.app.tokens.length ? state.app.tokens : (await getTokens()).tokens;
    const oneToken = tokens.find(
      (token) => token.symbol === search.token0 || token.address.toLowerCase() === search.token0.toLowerCase(),
    );
    const twoToken = tokens.find(
      (token) => token.symbol === search.token1 || token.address.toLowerCase() === search.token1.toLowerCase(),
    );
    const pool = await walletAdapterService!.getPool(oneToken!, twoToken!);
    const info = {
      token0PerToken1: new BigNumber(pool.input0.amount).div(new BigNumber(pool.input1.amount)).toString(),
      token1PerToken0: new BigNumber(pool.input1.amount).div(new BigNumber(pool.input0.amount)).toString(),
